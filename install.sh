#!/usr/bin/env bash
# AI Market Analyst — One-command installer
...
echo ""
echo "  ${BOLD}AI Market Analyst — Installer${NC}"
echo "  ${DIM}Real-time market intelligence with ML-powered analysis${NC}"
echo ""
...
INSTALL_DIR="$HOME/ai-market-analyst"
...
echo ""
echo "  ${BOLD}AI Market Analyst${NC}"
...
if [ "$IS_WSL" = false ] && [ -d "$HOME/.local/share/applications" ]; then
    cat > "$HOME/.local/share/applications/ai-market-analyst.desktop" << EOF
[Desktop Entry]
Name=AI Market Analyst
Comment=Real-time market intelligence with ML analysis
Exec=bash $LAUNCHER
Terminal=true
Type=Application
